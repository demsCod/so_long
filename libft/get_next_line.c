...
/*   Updated: 2024/07/08 19:47:50 by mdembele         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_clean(char *victim)
{
	...
	if (victim)
	{
		...
		ft_strlcpy(victim, victim + i + 1, ft_str_len(victim, '\0'));
	}
}
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, NULL, 0) < 0)
	{
		...
		{
			nextline = ft_split_strdup(stash);
			if (!nextline)
				return (NULL);
			return (nextline);
		}
